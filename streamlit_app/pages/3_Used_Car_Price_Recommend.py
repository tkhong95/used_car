...
def price_recommender(year,min_price,max_price,number_result):
    data = df.loc[(df['price']>=min_price) & (df['price']<=max_price) & (df['year']==year)]
...
st.write(price_recommender(input_feature['year'],input_feature['min_price'], input_feature['max_price'], 
